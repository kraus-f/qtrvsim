...
![alt text](images/gui_elements/registers_(in-box).svg)
- **Program** (Ctrl+P) – Code view with current instruction highlighting.  
![alt text](images/gui_elements/program_(in-box).svg)
- **Memory** (Ctrl+M) – Memory contents.
![alt text](images/gui_elements/memory_(in-box).svg)
- **Program Cache** (Ctrl+Shift+P) – Instruction cache visualization (if enabled).
![alt text](images/gui_elements/program_cache_(in-box).svg)
- **Data Cache** (Ctrl+Shift+M) – Data cache visualization (if enabled).
![alt text](images/gui_elements/data_cache_(in-box).svg)
- **L2 Cache** – Second-level cache view (if enabled).  
![alt text](images/gui_elements/l2_cache_(in-box).svg)
- **Branch Predictor** – Displays prediction tables and data (if configured).
![alt text](images/gui_elements/branch_predictor_(in-box).svg)
- **Peripherals** – Container for simulated I/O devices.
![alt text](images/gui_elements/peripherals_(in-box).svg)
- **Terminal** – Simulated serial terminal.  
![alt text](images/gui_elements/terminal_(in-box).svg)
- **LCD Display** – Simulated LCD. 
![alt text](images/gui_elements/lcd_display_(in-box).svg)
- **Control and Status Registers (CSR)** (Ctrl+I) – CSR panel.
![alt text](images/gui_elements/control_and_status_registers_(in-box).svg)
- **Core View** – Datapath visualization (CPU core).  
![alt text](images/gui_elements/core_(in-box).svg)
- **Messages** – Log and feedback panel.
![alt text](images/gui_elements/messages_(in-box).svg)
...
### Toolbar
![Toolbar screenshot](images/gui_elements/toolbar_(in-box).svg)
